...
public class CommentConverter {
  ...
  // CreateDto -> 엔티티
  public static Comment toEntity(CommentRequestDTO.CreateDto createDto, User user) {
    return Comment.builder()
        .context(createDto.getContext())
        .user(user) // User 엔티티는 서비스에서 받기
  ...
  public static CommentResponseDTO.ResponseDto toResponseDto(Comment comment) {
    return CommentResponseDTO.ResponseDto.builder()
        .id(comment.getId())
        .writerId(comment.getUser().getId()) // User 엔티티에서 작성자 ID를 가져옴
